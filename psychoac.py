...
class Masker:
    ...
    def vIntensityAtBark(self, zVec):
        ...
        spl = np.where(
            dz > 0.5, (-27 + 0.367 * max(self.SPL - 40, 0)) * (np.abs(dz) - 0.5), spl
        )
        spl = self.SPL + spl
        if self.isTonal:
            spl -= 16
...
def _compute_masking_curve(masker_bark_f, masker_spl_v, masker_tonal, bark_MDCT):
    ...
    upper_per_masker = -27.0 + 0.367 * np.maximum(masker_spl_v - 40.0, 0.0)
    upper = upper_per_masker.reshape(-1, 1)
    slope = np.where(dz > 0.5, upper * (abs_dz - 0.5), slope)

    total_spl = masker_spl_v.reshape(-1, 1) + slope

    # Bark-linear tonal/noise offset: ΔT(z) = a + b·z
    # TODO: Tweak later
    tonal_off = -16.0 + -0.0 * masker_bark_f
    noise_off = -6.0 + -0.0 * masker_bark_f
    offset = np.where(masker_tonal, tonal_off, noise_off).reshape(-1, 1)
    total_spl = total_spl + offset
...
def _build_psychoac_cache(N, N_mdct, sampleRate):
    ...
    intensity_quiet = Intensity(Thresh(MDCT_freqs))
...
def getMaskedThreshold(data, MDCTdata, MDCTscale, sampleRate, sfBands):
    ...
    peaks, _ = scipy.signal.find_peaks(x_spl, prominence=5.0)
    peak_amp_squared = (
        x_intensity[peaks - 1] + x_intensity[peaks] + x_intensity[peaks + 1]
    )
    ...
    # Zero a ±2-bin skirt around each peak. KBD α=4 mainlobe extends ±2 bins;
    # zeroing only ±1 left ~40 dB of mainlobe leakage in adjacent bands' noise
    # calc, inflating the noise masker. Width chosen empirically (skirt sweep).
    noise_intensity = np.copy(x_intensity)
    n_lines = len(noise_intensity)
    for s in (-2, -1, 0, 1, 2):
        idx = peaks + s
        idx = idx[(idx >= 0) & (idx < n_lines)]
        noise_intensity[idx] = 0

    idx = n_peaks
    for b in range(bands_fft.nBands):
        if band_valid[b]:
            noise_int = np.sum(
                noise_intensity[bands_fft.lowerLine[b] : bands_fft.upperLine[b] + 1]
            )
    ...
    total_spl = _compute_masking_curve(
        masker_bark_f, masker_spl, masker_tonal, bark_MDCT
    )

    # Aggregate all maskers w/ threshold in quiet via max aggregation
    masker_intensities = Intensity(total_spl)
    all_int = np.vstack([masker_intensities, intensity_quiet[np.newaxis, :]])
    combined = np.max(all_int, axis=0)

    masked_threshold = SPL(combined)
...
def CalcSMRs(data, MDCTdata, MDCTscale, sampleRate, sfBands):
    ...
    masked_threshold = getMaskedThreshold(
        data, MDCTdata, MDCTscale, sampleRate, sfBands
    )
