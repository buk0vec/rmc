    ...
    if codingParams.blockType == SHORT and features.AC2A_BLOCK_SWITCHING:
        ...
        for i in range(N_SHORT_BLOCKS):
            ...
            sf, ba, mant, ovs = (
                ...
                bitAlloc[i],
                mantissa[i],
                overallScaleFactor[i],
            )
            rescaleLevel = 1.0 * (1 << ovs)
            for iBand in range(sfBands_short.nBands):
                nLines = sfBands_short.nLines[iBand]
                if ba[iBand]:
                    mdctLine[iMant : (iMant + nLines)] = vDequantize(
                        sf[iBand],
                        mant[iMant : (iMant + nLines)],
            ...
            overlap_and_add[
                pad + i * halfN_short : pad + i * halfN_short + N_short
            ] += data
...
def Encode(data, codingParams, features: RMCFeatures):
    ...
    for iCh in range(codingParams.nChannels):
        ...
        (s, b, m, o) = EncodeSingleChannel(data[iCh], codingParams, features)
...
def EncodeSingleChannel(data, codingParams, features: RMCFeatures):
    ...
    if codingParams.blockType == SHORT and features.AC2A_BLOCK_SWITCHING:
        ...
        for i in range(N_SHORT_BLOCKS):
            ...
            mdctTimeSamples = (
                WindowForBlockType(codingParams.blockType, N_long, N_short)
                * timeSamples
            )
            mdctLines = MDCT(mdctTimeSamples, halfN, halfN)[:halfN]
            ...
            masking_samples = (
                codingParams._masking_signal[pad + i * halfN : pad + i * halfN + N]
        ...
        for G in group_lens:
            ...
            bitBudget -= (
                nScaleBits * sfBands_short.nBands
            )  # ONE set of band scale factors
            bitBudget -= (
                codingParams.nMantSizeBits * sfBands_short.nBands
            )  # ONE set of bit allocs
            bitBudget -= (
                codingParams._block_overhead * G / N_SHORT_BLOCKS
            )  # proportional share of bitstream overhead
            ...
            bitAlloc = BitAlloc(
                bitBudget,
                maxMantBits,
                sfBands_short.nBands,
                sfBands_short.nLines * G,
                combined_SMRs,
            )
            ...
            alloc_bands = [
                (
                    iBand,
                    int(sfBands_short.lowerLine[iBand]),
        ...
        bitBudget -= nScaleBits * (
            sfBands_long.nBands + 1
        )  # less scale factor bits (including overall scale factor)
        bitBudget -= (
            codingParams.nMantSizeBits * sfBands_long.nBands
        )  # less mantissa bit allocation bits
        bitBudget -= (
            codingParams._block_overhead
        )  # bitstream overhead (block type, pred, M/S, nBytes field)
        ...
        for iBand in range(sfBands_long.nBands):
            ...
            highLine = (
                sfBands_long.upperLine[iBand] + 1
